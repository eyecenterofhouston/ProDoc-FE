...
export class SidebarComponent implements OnInit, OnDestroy {
  ...
  filteredMenuItems(menuItems: IMenuItem[]): IMenuItem[] {
    ...
    return menuItems
      ? menuItems.filter( (x) => !x.roles || (x.roles && x.roles.includes(this.fillers(this.currentUser.roles,x.roles))) )
      : [];
  }

  fillers(userRole:any,appRoles:any){
    for(var i=0;i<userRole.length;i++){
      for(var j=0;j<appRoles.length;j++){
      if(userRole[i]==appRoles[j]){
        return userRole[i]
      }
      }
    }
